...
export class PikachuVolleyball {
  ...
  startOfNewGame() {
    if (this.frameCounter === 0) {
      ...
      this.physics.player1.initializeForNewRound(this.isPlayer2Serve);
      this.physics.player2.initializeForNewRound(this.isPlayer2Serve);
  ...
  round() {
    ...
    const playerTouchingBall = this.physics.runEngineForNextFrame(
      this.keyboardArray
    );
    ...
    if (
      playerTouchingBall > 0 &&
      this._isPracticeMode === false &&
      this.roundEnded === false &&
      this.gameEnded === false
    ) {
      if (playerTouchingBall == 1) {
