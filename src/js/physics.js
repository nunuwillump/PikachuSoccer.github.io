...
const GROUND_WIDTH = 640; // match with VIEWPORT_WIDTH in view.js
...
const GOAL_TOP_Y_COORD = VIEWPORT_HEIGHT - 152;
...
function isCollisionBetweenPlayers(player1, player2) {
    return (Math.abs(player1.x - player2.x) < (2 * PLAYER_HALF_LENGTH_X) && Math.abs(player1.y - player2.y) < (2 * PLAYER_HALF_LENGTH_Y));
}
...
function processCollisionBetweenPlayers(player1, player2) {
    if (2 * Math.abs(player1.x - player2.x) + 1 > Math.abs(player1.y - player2.y)) {
        let playerLeft;
        let playerRight;
        ...
        playerLeft.x = avgX - PLAYER_HALF_LENGTH_X;
        playerRight.x = avgX + PLAYER_HALF_LENGTH_X;
        if (playerLeft.x < PLAYER_HALF_LENGTH_X) {
            playerLeft.x = PLAYER_HALF_LENGTH_X;
            playerRight.x = 3 * PLAYER_HALF_LENGTH_X;
            player1.xVelocity = 0;
            player2.xVelocity = 0;
        } else if (playerRight.x > GROUND_WIDTH - PLAYER_HALF_LENGTH_X) {
            playerLeft.x = GROUND_WIDTH - 3 * PLAYER_HALF_LENGTH_X;
            playerRight.x = GROUND_WIDTH - PLAYER_HALF_LENGTH_X;
            player1.xVelocity = 0;
            player2.xVelocity = 0;
            ...
            playerRight.x = GOAL_TOP_WIDTH + 3 * PLAYER_HALF_LENGTH_X;
            player1.xVelocity = 0;
            player2.xVelocity = 0;
            ...
            playerLeft.x = GROUND_WIDTH - GOAL_TOP_WIDTH - 3 * PLAYER_HALF_LENGTH_X;
        ...
        if (playerTop.y < PLAYER_HALF_LENGTH_Y) {
            ...
            && playerTop.y > (GOAL_TOP_Y_COORD) && playerTop.y < (GOAL_TOP_Y_COORD + PLAYER_HALF_LENGTH_Y)
        ) {
            playerTop.y = GOAL_TOP_Y_COORD + PLAYER_HALF_LENGTH_Y;
            playerBottom.x = GOAL_TOP_Y_COORD + 3 * PLAYER_HALF_LENGTH_Y;
            player1.yVelocity = 0;
            player2.yVelocity = 0;
        } else if (
            (playerBottom.x < (GOAL_TOP_WIDTH + PLAYER_HALF_LENGTH_X) || playerBottom.x > (GROUND_WIDTH - GOAL_TOP_WIDTH - PLAYER_HALF_LENGTH_X))
            && playerBottom.y < (GOAL_TOP_Y_COORD) && playerBottom.y > (GOAL_TOP_Y_COORD - PLAYER_HALF_LENGTH_Y)
        ) {
            playerTop.y = GOAL_TOP_Y_COORD - 3 * PLAYER_HALF_LENGTH_Y;
            playerBottom.x = GOAL_TOP_Y_COORD - PLAYER_HALF_LENGTH_Y;
            player1.yVelocity = 0;
            player2.yVelocity = 0;
        }
    }
...
function processPlayerMovementAndSetPlayerPosition(
  ...
  if (player.state < 5 || player.state === 7) {
    if (player.state < 3) {
      playerVelocityX = userInput.xDirection * 12; // 6
    } else {
  ...
  if (Math.abs(player.y - GOAL_TOP_Y_COORD) < GOAL_TOP_HALF_HEIGHT + PLAYER_HALF_LENGTH_Y) {
      if (player.x < (GOAL_TOP_WIDTH + Math.abs(player.y - GOAL_TOP_Y_COORD) - GOAL_TOP_HALF_HEIGHT) || player.x > (GROUND_WIDTH - GOAL_TOP_WIDTH - Math.abs(player.y - GOAL_TOP_Y_COORD) + GOAL_TOP_HALF_HEIGHT)) {
          ...
          player.xVelocity = 0;
          if (player.x < GOAL_TOP_WIDTH + PLAYER_HALF_LENGTH_X) {
              player.x = GOAL_TOP_WIDTH + PLAYER_HALF_LENGTH_X;
          } else if (player.x > GROUND_WIDTH - GOAL_TOP_WIDTH - PLAYER_HALF_LENGTH_X) {
              player.x = GROUND_WIDTH - GOAL_TOP_WIDTH - PLAYER_HALF_LENGTH_X;
          }
      }
  }
