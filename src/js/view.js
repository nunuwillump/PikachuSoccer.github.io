 ...
 */
'use strict';
//import pkg from '../../../node_modules/pixi.js-legacy/lib/pixi-legacy.js';
//const { Container, Sprite, AnimatedSprite, Graphics } = pkg;
...
const NUM_OF_CLOUDS = 16;
/** @constant @type {number} width & height of viewport
 *  Important : also check index.js and physics.js and change values in them together */
...
export class IntroView {
  ...
  constructor(resources) {
      ...
      this.mark.x = VIEWPORT_WIDTH / 2;
      this.mark.y = VIEWPORT_HEIGHT / 2;
  ...
  drawMark(frameCounter) {
    ...
    if (frameCounter < 90) {
      mark.alpha = Math.min(1, mark.alpha + 1 / 30);
    } else if (frameCounter >= 90) {
      mark.alpha = Math.max(0, mark.alpha - 1 / 30);
    }
  }
}
...
export class MenuView {
  ...
  constructor(resources) {
    ...
    this.messages = {
      ...
      withWho: makeSpriteWithAnchorXY(textures, TEXURES.GAME_START, 0, 0),
    ...
    this.sittingPikachuTilesContainer = makeSittingPikachuTilesContainer(
      ...
      this.messages.sachisoft.x = VIEWPORT_WIDTH / 2 - this.messages.sachisoft.texture.width / 2;
      this.messages.sachisoft.y = VIEWPORT_HEIGHT - 40;
    ...
    this.container.addChild(this.messages.withWho);
  ...
  drawWithWhoMessages(frameCounter) {
    ...
    const w = withWho.texture.width;
    const h = withWho.texture.height;

    if (frameCounter === 0) {
      withWho.visible = false;
      return;
    }
    ...
    if (frameCounter > 70) {
      ...
      for (let i = 0; i < 1; i++) {
          ...
          withWho.visible = true;
          withWho.x = VIEWPORT_WIDTH / 2 - w / 2 - halfWidthIncrement;
          withWho.y = 184 - halfHeightIncrement;
          withWho.width = w + 2 * halfWidthIncrement;
          withWho.height = h + 2 * halfHeightIncrement;
      }
    }
  }
...
export class GameView {
  ...
  constructor(resources) {
    const textures = resources[ASSETS_PATH.SPRITE_SHEET].textures;
    const textures_2P = resources[ASSETS_PATH.SPRITE_SHEET_2P].textures;
    // Display objects below
    this.bgContainer = makeBGContainer(textures);
    const playerSprites = makePlayerAnimatedSprites(textures);
    const playerSprites_2P = makePlayerAnimatedSprites_2P(textures_2P);
    this.player1 = playerSprites[0];
    this.player2 = playerSprites_2P[0];
    ...
    this.container.addChild(this.messages.gameEnd);
      ...
      this.messages.ready.x = VIEWPORT_WIDTH / 2 - 40;
      this.messages.ready.y = 38;
      this.messages.deuce.x = VIEWPORT_WIDTH / 2 - 30;
      ...
      this.scoreBoards[1].x = VIEWPORT_WIDTH - 32 - 32 - 14; // 32 pixel is for number (32x32px) width; one score board has tow numbers
      this.scoreBoards[1].y = 10;

      this.shadows.forPlayer1.y = VIEWPORT_HEIGHT - 39;
      this.shadows.forPlayer2.y = VIEWPORT_HEIGHT - 39;
      this.shadows.forBall.y = VIEWPORT_HEIGHT - 39;
  ...
  drawPlayersAndBall(physics) {
    ...
    this.ball.gotoAndStop(ball.rotation);
      ...
      if (Math.abs(ball.x - ball.previousX) > 384) {
          this.ballHyper.x = ball.x;
          this.ballHyper.y = ball.y;
          this.ballTrail.x = ball.x;
          this.ballTrail.y = ball.y;
      } else {
      ...
      if (player1.holding || player2.holding) {
          this.ball.alpha = 0.5;
      } else {
          this.ball.alpha = 1.0;
      }
  ...
  drawGameStartMessage(frameCounter, frameTotal) {
    if (frameCounter === 0) {
      ...
      gameStartMessage.x = VIEWPORT_WIDTH / 2 - halfWidth;
  ...
  drawGameEndMessage(frameCounter) {
    ...
    if (frameCounter < 50) {
        ...
        gameEndMessage.x = VIEWPORT_WIDTH / 2 - w / 2 - halfWidthIncrement;
        ...
        gameEndMessage.x = VIEWPORT_WIDTH / 2 - w / 2;
        gameEndMessage.y = 50;
...
export class FadeInOut {
  constructor() {
      ...
      this.black.drawRect(0, 0, VIEWPORT_WIDTH, VIEWPORT_HEIGHT);
      this.black.endFill();
      this.black.x = 0;
      this.black.y = 0;
      this.black.alpha = 1;
...
function makeSittingPikachuTilesContainer(textures) {
  ...
  const h = texture.height;
    ...
    let tile;
    for (let j = 0; j < Math.floor(VIEWPORT_HEIGHT / h) + 2; j++) {
        for (let i = 0; i < Math.floor(VIEWPORT_WIDTH / w) + 2; i++) {
...
function makeBGContainer(textures) {
    ...
    for (let j = 0; j < (VIEWPORT_HEIGHT - 128) / 16; j++) {
        for (let i = 0; i < VIEWPORT_WIDTH / 16; i++) {
            tile = new Sprite(texture);
    ...
    for (let i = 0; i < 2; i++) {
        tile = new Sprite(texture);
        addChildToParentAndSetLocalPosition(bgContainer, tile, VIEWPORT_WIDTH / 2 - (1 - i) * 432, VIEWPORT_HEIGHT - 128);
    } 
    ...
    for (let i = 0; i < VIEWPORT_WIDTH / 16; i++) {
        tile = new Sprite(texture);
        addChildToParentAndSetLocalPosition(bgContainer, tile, 16 * i, VIEWPORT_HEIGHT - 64);
    }

    // ground_line
    texture = textures[TEXURES.GROUND_LINE];
    for (let i = 1; i < VIEWPORT_WIDTH / 16 - 1; i++) {
    tile = new Sprite(texture);
        addChildToParentAndSetLocalPosition(bgContainer, tile, 16 * i, VIEWPORT_HEIGHT - 48);
    }
    texture = textures[TEXURES.GROUND_LINE_LEFT_MOST];
    tile = new Sprite(texture);
    addChildToParentAndSetLocalPosition(bgContainer, tile, 0, VIEWPORT_HEIGHT - 48);
    texture = textures[TEXURES.GROUND_LINE_RIGHT_MOST];
    tile = new Sprite(texture);
    addChildToParentAndSetLocalPosition(bgContainer, tile, VIEWPORT_WIDTH - 16, VIEWPORT_HEIGHT - 48);
    ...
    for (let j = 0; j < 2; j++) {
        for (let i = 0; i < VIEWPORT_WIDTH / 16; i++) {
            ...
            addChildToParentAndSetLocalPosition(
                bgContainer, tile, 16 * i, VIEWPORT_HEIGHT - 32 + 16 * j
            );
        }
    }

    // net pillar - used as goal posts in this case
    texture = textures[TEXURES.NET_PILLAR_TOP];
    tile = new Sprite(texture);
    addChildToParentAndSetLocalPosition(bgContainer, tile, 0, VIEWPORT_HEIGHT - 152);
    texture = textures[TEXURES.NET_PILLAR];
    for (let j = 0; j < 14; j++) {
        tile = new Sprite(texture);
        addChildToParentAndSetLocalPosition(bgContainer, tile, 0, VIEWPORT_HEIGHT - 144 + 8 * j);
    }
    texture = textures[TEXURES.NET_PILLAR_TOP];
    tile = new Sprite(texture);
    addChildToParentAndSetLocalPosition(bgContainer, tile, VIEWPORT_WIDTH - 8, VIEWPORT_HEIGHT - 152);
    texture = textures[TEXURES.NET_PILLAR];
    for (let j = 0; j < 14; j++) {
        tile = new Sprite(texture);
        addChildToParentAndSetLocalPosition(bgContainer, tile, VIEWPORT_WIDTH - 8, VIEWPORT_HEIGHT - 144 + 8 * j);
    }

    texture = textures[TEXURES.NET_PILLAR_ROTATE];
    for (let i = 0; i < 7; i++) {
        tile = new Sprite(texture);
        addChildToParentAndSetLocalPosition(bgContainer, tile, VIEWPORT_WIDTH - 64 + 8 * i, VIEWPORT_HEIGHT - 152);
    }
    for (let i = 0; i < 7; i++) {
        tile = new Sprite(texture);
        addChildToParentAndSetLocalPosition(bgContainer, tile, 8 + 8 * i, VIEWPORT_HEIGHT - 152);
    }
    return bgContainer;
}
...
function makePlayerAnimatedSprites_2P(textures_2P) {
  const getPlayerTexture = (i, j) => textures_2P[TEXTURES_2P.PIKACHU(i, j)];
  const playerTextureArray = [];
  for (let i = 0; i < 7; i++) {
    if (i === 3) {
      playerTextureArray.push(getPlayerTexture(i, 0));
